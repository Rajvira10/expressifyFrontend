...
const ManageLearningTracks: FC<ManageLearningTracksProps> = ({ id }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axios.get(
        Routes.GET_LEARNING_TRACKS_BY_COURSE(id),
        {
          headers: {
  ...
  const { mutate: courseMutation, isPending } = useMutation({
    mutationFn: async (data: FormFields) => {
      const response = await axios.post(
        Routes.ADD_LEARNING_TRACK_TO_COURSE(id),
        data,
        {
          headers: {
            Authorization: `Bearer ${adminToken}`,
  ...
  const { mutate: removeTrackMutation } = useMutation({
    mutationFn: async (trackId: number) => {
      const response = await axios.post(
        Routes.REMOVE_LEARNING_TRACK_FROM_COURSE(id),
        {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Add a learning track to this course
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
