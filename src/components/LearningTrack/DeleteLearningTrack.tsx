...
interface DeleteLearningTrackProps {
  id: number;
  onClose: () => void;
}

const DeleteLearningTrack: FC<DeleteLearningTrackProps> = ({ id, onClose }) => {
  ...
  const { mutate: trackMutation, isPending } = useMutation({
    mutationFn: async () => {
      const response = await axios.post(
        Routes.DELETE_LEARNING_TRACK(id),
        {},
        {
          headers: {
...
export default DeleteLearningTrack;
