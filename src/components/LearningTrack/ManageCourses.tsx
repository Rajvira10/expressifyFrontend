...
const ManageCourses: FC<ManageCoursesProps> = ({ id }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axios.get(
        Routes.GET_COURSES_BY_LEARNING_TRACK(id),
        {
          headers: {
  ...
  const { mutate: courseMutation, isPending } = useMutation({
    mutationFn: async (data: FormFields) => {
      const response = await axios.post(
        Routes.ADD_COURSE_TO_LEARNING_TRACK(id),
        data,
        {
          headers: {
            Authorization: `Bearer ${adminToken}`,
  ...
  const { mutate: removeCourseMutation } = useMutation({
    mutationFn: async (courseId: number) => {
      const response = await axios.post(
        Routes.REMOVE_COURSE_FROM_LEARNING_TRACK(id),
        {
          course_id: courseId,
        },
        {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Add a course to this learning track
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
